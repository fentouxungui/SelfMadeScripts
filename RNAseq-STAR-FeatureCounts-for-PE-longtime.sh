...
print_usage_and_exit(){
    echo "Usage: $0 [-f <path of fastq files> ] [-j <number of threads> =24 default] [-m <Galore minimum length> =$length] -i <star index> =path/to/STAR/index -b <genes bed file> =path/to/genes.bed -g <genes gtf file> =path/to/genes.gtf "
    exit 1
}

while getopts ":f:j:m:i:b:g:h:" opt; do
    case $opt in
        f)
            fastq="$OPTARG"
