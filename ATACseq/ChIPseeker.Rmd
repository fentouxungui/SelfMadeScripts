...
peak_list = list()
macs2.path <- "../2_macs2"
peaks.files <- grep("_peaks.narrowPeak$",list.files(macs2.path),value = TRUE)

for (peaks.file in peaks.files) {
  extraCols_narrowPeak <- c(signalValue = "numeric", pValue = "numeric", qValue = "numeric", peak = "integer")
  gr_narrowPeak <- import(paste(macs2.path,peaks.file,sep = "/"), format = "BED",extraCols = extraCols_narrowPeak)
  peak_list[gsub("_peaks.narrowPeak","",peaks.file)] <- gr_narrowPeak
}
```
...
lapply(peaks.samples, function(x){
  ...
  write.table(as.data.frame(get(x)), paste(anno.dir,"/",x,".anno.txt", sep = ""),quote=F, row.names=F, sep="\t")
})
...
```{r}
files.txt <- list.files("./Annotation/",pattern = "txt$",full.names = TRUE)
...
```{r}
df.list <- lapply(anno_list, function(x)as.data.frame(x)[,c("geneId", "transcriptId", "ENTREZID", "SYMBOL", "GENENAME", "seqnames", "name", "distanceToTSS",  "annotation",  "score", "signalValue", "pValue", "qValue")])

library(dplyr)
lapply(df.list, function(x){
  x %>%
  group_by(geneId, ENTREZID, SYMBOL, GENENAME, seqnames) %>% 
  summarize(name = paste(name, collapse = ";"),
            ...
            signalValue = paste(signalValue, collapse = ";"),
            pValue = paste(pValue, collapse = ";"),
            qValue = paste(qValue, collapse = ";"))
}) -> df.list
...
#   }else{
#     message("More than 3000 genes are found! the top 3000 scored genes will be output!")
#     return(as.data.frame(i)$geneId[order(as.data.frame(i)$score,decreasing = TRUE)][1:3000])
#   }
# })
# 
# #names(genes) = sub("_", "\n", names(genes))
