...
countdata <- read.table("../6_final_counts/final_counts.txt", header = TRUE, skip = 1, row.names = 1)
head(countdata)
```
...
colnames(countdata) <- substring(colnames(countdata),1,2)
```
...
metadata <- data.frame(row.names = colnames(countdata),
                       Group=c(rep("R",4),rep("F",4)),
                       Replicate=c(paste("Rep",1:4,sep = ""),paste("Rep",1:4,sep="")),
...
plotPCA(ddsMat_rlog, intgroup = "Group", ntop = 500) +
  ...
  scale_y_continuous(limits = c(-40, 40)) + # change limits to fix figure dimensions
...
mat <- assay(ddsMat_rlog[row.names(results_sig[order(results_sig$padj),])])[1:72, ]
...
ann_colors = list(
  Group = c(R = "lightblue", F = "darkorange"),
  Replicate = c(Rep1 = "darkred", Rep2 = "forestgreen",Rep3 = "darkblue",Rep4 = "gray")
)
...
vol +   
  ...
  xlab(expression(log[2]("ProsIR" / "EE"))) + # Change X-Axis label
...
for (genegroup in c("total","upregulated","downregulated")) {
  ...
  write.csv(go_enrich,file = paste("GO-Biological-Process-using",genegroup,"Genes.csv",sep = "-"))
  #write.csv(go_enrich,file = "go_enrich_results_using_upregulated_genes.csv")
  #write.csv(go_enrich,file = "go_enrich_results_using_downregulated_genes.csv")
}
...
for (genegroup in c("total","upregulated","downregulated")) {
  ...
  print(barplot(kegg_enrich, 
          ...
          font.size = 8))
  write.csv(kegg_enrich,file = paste("KEGG-Enrichment-Pathways",genegroup,"Genes.csv",sep = "-"))
}
```

## Step 11. Plotting KEGG Pathways (using total DEGs!)
...
for (path_id in kegg_enrich@result$ID[kegg_enrich@result$p.adjust < 0.05]) {
  suppressMessages(pathview(gene.data = gene_matrix, 
         pathway.id = path_id, 
         species = "mmu"))
}
```

## step 12, GSEA analysis

```{r}
# export rnk list using ENSEMBL gene ids
geneList = results[,"log2FoldChange"]
names(geneList) <- names(results)
geneList = sort(geneList, decreasing = TRUE)
write.table(geneList,file = "RNAseq-GeneNameENSEMBL-RankedBy-log2FoldChange.rnk",quote = FALSE,col.names = FALSE,sep = "\t")
```
...
geneList = sort(geneList, decreasing = TRUE)
write.table(geneList,file = "RNAseq-GeneNameENTREZID-RankedBy-log2FoldChange.rnk",quote = FALSE,col.names = FALSE,sep = "\t")
```

### 12.1 Using ClusterProfiler
...
if (dim(ego2@result)[1] != 0) {
  print(enrichplot::gseaplot2(ego2, geneSetID = 1,pvalue_table = TRUE))
  write.csv(ego2@result,file = "gseGO-CC.csv")
}
```
...
if (dim(ego3@result)[1] != 0) {
  print(enrichplot::gseaplot2(ego3, geneSetID = 1,pvalue_table = TRUE))
  write.csv(ego3@result,file = "gseGO-BP.csv")
}
```
...
if (dim(kk2@result)[1] != 0) {
  print(enrichplot::gseaplot2(kk2, geneSetID = 1,pvalue_table = TRUE))
  write.csv(kk2@result,file = "gseKEGG.csv")
}
```
#### 12.1.3  MSigDb analysis Only for Human Genes! 
...
#### 12.1.4  Using your own gene sets
...
if (FALSE) {
  ...
  write.table(t(c("oDAM-Pros-peaks","GuoXingting",
                  # make sure using same type gene ids(ENSEMBL or ENTREZID) !
                  unique(odam$Gene.Name[odam$Gene.Name%in% names(geneList)]))),
...
### 12.2 Using GSEA software in windows
...
write.table(x = counts(ddsMat, normalized = T), 
            file = 'normalized_counts_for_GSEA.txt', 
            sep = '\t', 
            quote = F, 
            col.names = NA)
```
