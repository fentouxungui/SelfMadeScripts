...
```{r}
AnalysisDirs <- list.dirs("../../Single-Sample/Cellbender-Seurat-Scrublet",recursive = FALSE,full.names = TRUE)
rds.files <- list.files(paste(AnalysisDirs,"/3.3-Clean-Cells-Rds",sep = ""),pattern = "rds$",full.names = TRUE)
rds.files <- grep("Rectum",grep("Mesen",rds.files,value = TRUE),invert = TRUE,value = TRUE)
basename(rds.files)
```
...
```{r}
# remove sample: Tu-mesen-jeju.doublets.removed.rds
# obj.list <- obj.list[-6]
```
...
immune.combined$short.name <- gsub("APCmin_Mouse_","",immune.combined@meta.data$orig.ident)
VlnPlot(immune.combined, features = qc.keys, ncol = 4,group.by = "short.name",pt.size = 0.01)
...
VlnPlot(immune.combined,features = c("Xist"),group.by = "short.name",pt.size = 0.001)
```

# Add Sample Annotations

```{r}
immune.combined@meta.data$type <-  unlist(lapply(strsplit(immune.combined@meta.data$orig.ident,split = "_"),"[",4))
immune.combined@meta.data$sex <- ifelse(immune.combined@meta.data$orig.ident == "APCmin_Mouse_duo_Tumor_Mesen_200421","male","female+male")
immune.combined@meta.data$location <- unlist(lapply(strsplit(immune.combined@meta.data$orig.ident,split = "_"),"[",3))

table(immune.combined@meta.data$orig.ident)
table(immune.combined@meta.data$type)
table(immune.combined@meta.data$location)
...
```{r}
colnames(immune.combined@meta.data)[grepl("(_snn_res)|(^Annotation)",colnames(immune.combined@meta.data))] <- 
  paste("Single_",colnames(immune.combined@meta.data)[grepl("(_snn_res)|(^Annotation)",colnames(immune.combined@meta.data))],sep = "")
```
...
```{r fig.width=6,fig.height=5}
immune.combined <- RunHarmony(immune.combined, group.by.vars = c("orig.ident","sex","location"), assay = DefaultAssay(immune.combined))
...
clustree(dplyr::filter(immune.combined@meta.data,type == "Para"),prefix = paste(DefaultAssay(immune.combined),"_snn_res.",sep = ""))
clustree(dplyr::filter(immune.combined@meta.data,type == "Tumor"),prefix = paste(DefaultAssay(immune.combined),"_snn_res.",sep = ""))
```
...
DimPlot(immune.combined,group.by = "type")
...
for (i in levels(immune.combined@active.ident)) {
  tmp <- subset(immune.combined,idents = i)
  p1 <- plotHighestExprs(as.SingleCellExperiment(subset(tmp, type == "Para"),assay = DefaultAssay(immune.combined)),colour_cells_by = cluster.resolution)
  p2 <- plotHighestExprs(as.SingleCellExperiment(subset(tmp, type == "Tumor"),assay = DefaultAssay(immune.combined)),colour_cells_by = cluster.resolution)
...
```{r}
expr.ave <- AverageExpression(immune.combined, assays = DefaultAssay(immune.combined), add.ident = "type")[[1]]
expr.ave$gene <- rownames(expr.ave)
expr.ave.melt <- reshape2::melt(expr.ave,id.vars = "gene", value.name = "expr",variable.name = "cluster")
top_20_genes <- expr.ave.melt %>%
  group_by(cluster) %>%
...
markers <- c("Pdgfra","Epcam","Ptprc","Vwf","Pdgfrb","Nrxn1",'Rgs5','Acta2','Cd34','Il33','Il1rl1')

VlnPlot(immune.combined,features = qc.keys, pt.size = 0, ncol = 3) + NoLegend()
VlnPlot(subset(immune.combined, type == "Para"),features = qc.keys, pt.size = 0, ncol = 3) + NoLegend()
VlnPlot(subset(immune.combined, type == "Tumor"),features = qc.keys, pt.size = 0, ncol = 3) + NoLegend()

VlnPlot(immune.combined,features = markers, pt.size = 0, ncol = 3) + NoLegend()
VlnPlot(subset(immune.combined, type == "Para"), features = markers, pt.size = 0, ncol = 3) + NoLegend()
VlnPlot(subset(immune.combined, type == "Tumor"), features = markers, pt.size = 0, ncol = 3) + NoLegend()
```

```{r fig.height=10,fig.width=10}
VlnPlot(immune.combined,features = qc.keys, pt.size = 0, ncol = 3, group.by = "type") + NoLegend()
VlnPlot(immune.combined,features = qc.keys, group.by = "type",pt.size = 0)  + NoLegend()
```
