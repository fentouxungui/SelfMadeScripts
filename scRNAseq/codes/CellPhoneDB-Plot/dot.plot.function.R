...
dot_plot(selected_rows = Para.mean$interacting_pair[Para.mean$id_cp_interaction %in% Pairs.seleted.final],
         selected_columns = gsub(".","|",grep("hEpi",colnames(Para.mean),value = TRUE),fixed = TRUE),
         filename = 'plot.pdf',
         width = 8,
         height = 10,
...
means_separator = '\t'
pvalues_separator = '\t'
all_pval = read.table('../ParaCells/pvalues.txt', header=T, stringsAsFactors = F, sep=means_separator, comment.char = '', check.names=F)
all_means = read.table('../ParaCells/means.txt', header=T, stringsAsFactors = F, sep=pvalues_separator, comment.char = '', check.names=F)
