...
```{r init}
# conda activate /home/xilab/software/miniconda-envs/seurat-latest # in shell
library(Seurat)
library(SeuratData)
...
library(patchwork)
library(qs2)
options(future.globals.maxSize = 10 * 1024 ^ 3) # max 10GB
```
...
```{r}
obj <- qs_read('../../merged.qs2')
obj
```

```{r}
colnames(obj@meta.data)[grepl('(^cca)|(^scvi)|(^harmony)|(^mnn)|(^rpca)', colnames(obj@meta.data))] <- paste0("All_", colnames(obj@meta.data)[grepl('(^cca)|(^scvi)|(^harmony)|(^mnn)|(^rpca)', colnames(obj@meta.data))])
```

```{r}
Idents(obj) <- 'All_cca_clusters_res_1'
DimPlot(obj, reduction = 'umap.cca', label = TRUE)
```
```{r}
obj <- subset(obj, idents = c(12, 5, 21, 17, 10, 16, 29, 25, 24, 30, 31))
obj
```
...
```{r}
obj <- DietSeurat(obj, dimreducs = NULL)
```
