---
title: "Seurat for Single Sample"
...
```{r}
# 设置物种名字，用于识别线粒体基因，计算线粒体基因的表达占比。
mt.species <- c(mouse = "^Mt:", fly = "^mt:", human = "^MT-")
species <- "human"
```
...
cellranger_mat <- Read10X(data.dir = "../../../matrix/2403067_ST_lL972/filtered_feature_bc_matrix/")
...
pbmc[["percent.mt"]] <- PercentageFeatureSet(pbmc, pattern = unname(mt.species[species]))
...
VlnPlot(pbmc, features = c("nFeature_RNA", "nCount_RNA", "percent.mt", "log10GenesPerUMI"), ncol = 4) &   theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank())
```
...
FeaturePlot(pbmc, features = c("nFeature_RNA", "nCount_RNA", "percent.mt", "log10GenesPerUMI"), reduction = "umap") 
FeaturePlot(pbmc, features = c("nFeature_RNA", "nCount_RNA", "percent.mt", "log10GenesPerUMI"), reduction = "tsne") 
```
