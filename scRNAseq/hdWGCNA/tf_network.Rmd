...
species <- 'mouse' # human, mouse or fly
```
...
if (species == 'human') {
  ...
  stop('Not ready for fly')
}else{
  stop('Please input correct species from human, mouse and fly!')
}
...
seurat_obj <- SetDatExpr(seurat_obj, group.by = group_by,
...
tf_regulons <- GetTFRegulons(seurat_obj, data_short_name)
plot_df <- tf_regulons %>%  mutate(score = Gain * sign(Cor))  %>% subset(abs(score) >  0.2)
print(sort(table(plot_df$tf)))

p1 <- RegulonBarPlot(seurat_obj, selected_tf='Smad4', cutoff = 0.2)
p2 <- RegulonBarPlot(seurat_obj, selected_tf='Nfe2l2', cutoff = 0.2)
p1 | p2
```
...
print(colnames(pos_regulon_scores))
print(colnames(neg_regulon_scores))
...
if (nrow(marker_tfs) != 0) {
  # get top 25 TFs
  # top_tfs <- marker_tfs %>% subset(cluster == 'INH') %>% slice_max(n=25, order_by=avg_log2FC)
  top_tfs <- marker_tfs %>% slice_max(n=25, order_by=avg_log2FC)
  
  # intersect marker TFs and hub genes:
  print(intersect(top_tfs$gene, hub_df$gene_name))
}
```
...
```{r eval=TRUE, fig.width=16,fig.height=4}
...
gene_depths <- cur_network %>% 
    ...
    ungroup() %>%
    dplyr::select(c(gene, depth)) %>% distinct()
