...
description: Tutorial for applying the core functions of hdWGCNA in single-cell data.
---
...
seurat_obj <- readRDS('../subset-pdgfra-from-adult-SI-Crypt.rds')

# self added
group_by <- 'RNA_snn_res.0.4'
data_name <- 'Pdgfra of mouse SI Crypt by Jiang'
data_short_name <- 'pdgfra'

seurat_obj@meta.data[group_by] <- as.character(seurat_obj@meta.data[group_by])
```
...
DimPlot(seurat_obj, group.by=group_by, label=TRUE) +
  ggtitle(data_name) +
  NoLegend()
```
...
seurat_obj <- SetupForWGCNA(
  ...
  wgcna_name = data_short_name # the name of the hdWGCNA experiment
)
```
...
#   ident.group = group_by # set the Idents of the metacell seurat object
# )
...
# p1 <- DimPlotMetacells(seurat_obj, group.by=group_by) + umap_theme() + ggtitle("Cell Type")
...
seurat_obj <- SetDatExpr(
  seurat_obj,
  group_name = unique(seurat_obj@misc[[data_short_name]]$wgcna_metacell_obj@meta.data[group_by]),
  group.by = group_by,
  assay = 'RNA', # using RNA assay
  layer = 'data' # using normalized data
)
...
seurat_obj <- ConstructNetwork(
  seurat_obj,
  tom_name = data_short_name, # name of the topoligical overlap matrix written to disk
  overwrite_tom = TRUE
)
...
PlotDendrogram(seurat_obj, main = paste0(data_short_name, ' hdWGCNA Dendrogram'))
```
...
write.csv(hub_df, file = 'modules-top10-hub-genes.csv')
```
...
# seurat_obj$cluster <- do.call(rbind, strsplit(as.character(seurat_obj@meta.data[group_by]), ' '))[,1]
# ModuleRadarPlot(
#   seurat_obj,
#   group.by = 'cluster',
...
DotPlot(seurat_obj, features=mods, group.by = group_by) +
  RotatedAxis() +
