...
```{r}
cds.mat <- Read10X("../../../CellRanger-Outputs/esg_RFP_Neg/outs/filtered_feature_bc_matrix/")
...
Attention, some cells has zero CellBender revised UMI counts. This is a bug in cellbender step, while no effect on cellranger ouputs.
...
```{r}
cds.sub <- subset(cds, subset =  nCount_RNA > 300)
cds.sub <- subset(cds.sub, subset =  nCount_RNA < 3000)
cds.sub
```
...
p3 <- metadata %>% 
  	ggplot(aes(color=Keep, x=nCount_RNA_RemoveMT, fill= Keep)) + 
  	geom_density(alpha = 0.2) + 
  	scale_x_log10() + 
  	theme_classic() +
  	ylab("Cell density") +
...
```{r fig.width=10,fig.height=12}
qc.keys <- colnames(cds@meta.data)[2:6]
VlnPlot(cds,features = qc.keys, ncol = 3,pt.size = 0.001,group.by = "orig.ident") + NoLegend()
VlnPlot(cds,features = qc.keys, ncol = 3,pt.size = 0,group.by = "orig.ident") + NoLegend()
```
...
```{r}
s.genes <- readRDS("~/reference/Biology-Database/Cell-Cycles-Genes/Fly/S.Genes.Symbol.rds")
g2m.genes <- readRDS("~/reference/Biology-Database/Cell-Cycles-Genes/Fly/G2M.Genes.Symbol.rds")
s.genes
g2m.genes
```

```{r}
table(s.genes %in% rownames(cds))
table(g2m.genes %in% rownames(cds))
```
...
writeMM(Matrix(Amatrix,sparse = TRUE),file = paste(Output.Rds.dir,basename(getwd()),".UMI.mtx",sep = ""))
```
