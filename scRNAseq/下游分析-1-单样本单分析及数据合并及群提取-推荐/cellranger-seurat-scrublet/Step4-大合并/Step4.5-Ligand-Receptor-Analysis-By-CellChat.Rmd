...
Results.dir <- "./4.5-Ligand-Receptor-Analysis/CellChat"
```
...
```{r}
# # Chord diagram
# group.cellType <- c(rep("Mesen", 4), rep("DC", 4), rep("TC", 4)) # grouping cell clusters into fibroblast, DC and TC cells
...
netVisual_bubble(cellchat, sources.use = 5, targets.use = 7, remove.isolate = FALSE)
```


```{r}
# show all the significant interactions (L-R pairs) associated with certain signaling pathways
netVisual_bubble(cellchat, sources.use = 7, targets.use = 5, signaling = c("CCL","CXCL"), remove.isolate = FALSE)
```
...
netVisual_bubble(cellchat, sources.use = 1, targets.use = 2, pairLR.use = pairLR.use, remove.isolate = TRUE)
...
netVisual_chord_gene(cellchat, sources.use = 1, targets.use = 2, lab.cex = 1,legend.pos.y = 30, big.gap = 20)
```
...
```{r}
saveRDS(cellchat, file = paste0(Results.dir,"/", "Embryo-D145-CellChat.rds"))
```
