...
```{r}
library(Seurat)
epi <- readRDS("../Subset/Subset-Epi-No-Intergration/4.1-Merged-Rds/subset-epi.rds")
mesen <- readRDS("../Subset/Subset-Mesen-And-EndoEpi-Remove-Contaminants/4.1-Merged-Rds/Merge-All-Embryo.rds")
cds <- merge(epi,mesen)
Idents(cds) <- "Annotation.celltype"
```
...
count_raw <- as.matrix(cds@assays$RNA@counts)
nor.umi <- apply(count_raw, 2, function(x) (x/sum(x))*10000)
sample.meta <- data.frame(Cell = colnames(cds), cell_type = cds@meta.data$Annotation.celltype)
```
