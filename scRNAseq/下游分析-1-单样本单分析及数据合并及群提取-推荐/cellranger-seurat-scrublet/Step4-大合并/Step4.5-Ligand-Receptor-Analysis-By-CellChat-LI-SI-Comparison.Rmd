...
```{r}
cellchat.LI <- readRDS("./4.5-Ligand-Receptor-Analysis/CellChat/Subset-LI/PostNatal-D21-LI-CellChat.rds")
cellchat.SI <- readRDS("./4.5-Ligand-Receptor-Analysis/CellChat/Subset-SI/PostNatal-D21-SI-CellChat.rds")
object.list <- list(LI = cellchat.LI, SI = cellchat.SI)
...
group.cellType <- gsub("-\\d+","",levels(cellchat@idents$joint)) # grouping cell clusters into fibroblast, DC and TC cells
names(group.cellType) <- levels(cellchat@idents$joint)
...
gg1 <- netAnalysis_signalingChanges_scatter(cellchat, idents.use = "EntericNeuron")
gg2 <- netAnalysis_signalingChanges_scatter(cellchat, idents.use = "Endothelial-1")
gg3 <- netAnalysis_signalingChanges_scatter(cellchat, idents.use = "Endothelial-2")
patchwork::wrap_plots(plots = list(gg1,gg2,gg3))
```
```{r fig.width=18,fig.height=4}
gg1 <- netAnalysis_signalingChanges_scatter(cellchat, idents.use = "Immune-1")
gg2 <- netAnalysis_signalingChanges_scatter(cellchat, idents.use = "Immune-2")
gg3 <- netAnalysis_signalingChanges_scatter(cellchat, idents.use = "Immune-3")
patchwork::wrap_plots(plots = list(gg1,gg2,gg3))
```
```{r fig.width=12,fig.height=4}
gg1 <- netAnalysis_signalingChanges_scatter(cellchat, idents.use = "Mesen-1")
gg2 <- netAnalysis_signalingChanges_scatter(cellchat, idents.use = "Mesen-2")
patchwork::wrap_plots(plots = list(gg1,gg2))
```
```{r fig.width=18,fig.height=4}
gg1 <- netAnalysis_signalingChanges_scatter(cellchat, idents.use = "Epithelial-1")
gg2 <- netAnalysis_signalingChanges_scatter(cellchat, idents.use = "Epithelial-2")
gg3 <- netAnalysis_signalingChanges_scatter(cellchat, idents.use = "Epithelial-3")
patchwork::wrap_plots(plots = list(gg1,gg2,gg3))
```
...
```{r fig.width=8,fig.height=8}
ht1 = netAnalysis_signalingRole_heatmap(object.list[[i]], pattern = "incoming", signaling = pathway.union, title = names(object.list)[i], width = 6, height = 12, color.heatmap = "GnBu")
ht2 = netAnalysis_signalingRole_heatmap(object.list[[i+1]], pattern = "incoming", signaling = pathway.union, title = names(object.list)[i+1], width = 6, height = 12, color.heatmap = "GnBu")
draw(ht1 + ht2, ht_gap = unit(0.5, "cm"))
```


```{r fig.width=8,fig.height=8}
ht1 = netAnalysis_signalingRole_heatmap(object.list[[i]], pattern = "all", signaling = pathway.union, title = names(object.list)[i], width = 6, height = 12, color.heatmap = "OrRd")
ht2 = netAnalysis_signalingRole_heatmap(object.list[[i+1]], pattern = "all", signaling = pathway.union, title = names(object.list)[i+1], width = 6, height = 12, color.heatmap = "OrRd")
draw(ht1 + ht2, ht_gap = unit(0.5, "cm"))
```
...
idents.Immune <- which(grepl("^Immune",levels(cellchat@idents$joint)))
idents.Epi <- which(grepl("^Epithelial",levels(cellchat@idents$joint)))

netVisual_bubble(cellchat, sources.use = idents.Epi, targets.use = idents.Immune,  comparison = c(1, 2), angle.x = 45)
#> Comparing communications on a merged object
```
```{r fig.width=6,fig.height=3}
netVisual_bubble(cellchat, sources.use = idents.Immune, targets.use = idents.Epi,  comparison = c(1, 2), angle.x = 45)
#> Comparing communications on a merged object
```
...
#gg1 <- netVisual_bubble(cellchat, sources.use = idents.Immune, targets.use = idents.Epi,  comparison = c(1, 2), max.dataset = 2, title.name = "Increased signaling in SI", angle.x = 45, remove.isolate = T)
#> Comparing communications on a merged object
gg2 <- netVisual_bubble(cellchat, sources.use = idents.Immune, targets.use = idents.Epi,  comparison = c(1, 2), max.dataset = 1, title.name = "Decreased signaling in SI", angle.x = 45, remove.isolate = T)
#> Comparing communications on a merged object
#gg1 + gg2
gg2
```
...
```{r fig.width=10, fig.height=4}
gg1 <- netVisual_bubble(cellchat, sources.use = idents.Epi, targets.use = idents.Immune,  comparison = c(1, 2), max.dataset = 2, title.name = "Increased signaling in SI", angle.x = 45, remove.isolate = T)
#> Comparing communications on a merged object
gg2 <- netVisual_bubble(cellchat, sources.use = idents.Epi, targets.use =idents.Immune,  comparison = c(1, 2), max.dataset = 1, title.name = "Decreased signaling in SI", angle.x = 45, remove.isolate = T)
#> Comparing communications on a merged object
gg1 + gg2
```
...
net.up <- subsetCommunication(cellchat, net = net, datasets = "SI",ligand.logFC = 0.2, receptor.logFC = NULL)
# extract the ligand-receptor pairs with upregulated ligands and upregulated recetptors in NL, i.e.,downregulated in LS
net.down <- subsetCommunication(cellchat, net = net, datasets = "LI",ligand.logFC = -0.1, receptor.logFC = -0.1)
```
...
gg1 <- netVisual_bubble(cellchat, pairLR.use = pairLR.use.up, sources.use = idents.Epi, targets.use = idents.Immune, comparison = c(1, 2),  angle.x = 90, remove.isolate = T,title.name = paste0("Up-regulated signaling in ", names(object.list)[2]))
#> Comparing communications on a merged object
pairLR.use.down = net.down[, "interaction_name", drop = F]
gg2 <- netVisual_bubble(cellchat, pairLR.use = pairLR.use.down, sources.use = idents.Epi, targets.use = idents.Immune, comparison = c(1, 2),  angle.x = 90, remove.isolate = T,title.name = paste0("Down-regulated signaling in ", names(object.list)[2]))
#> Comparing communications on a merged object
gg1 + gg2
```
```{r}
pairLR.use.up = net.up[, "interaction_name", drop = F]
gg1 <- netVisual_bubble(cellchat, pairLR.use = pairLR.use.up, sources.use = idents.Immune, targets.use = idents.Epi, comparison = c(1, 2),  angle.x = 90, remove.isolate = T,title.name = paste0("Up-regulated signaling in ", names(object.list)[2]))
#> Comparing communications on a merged object
pairLR.use.down = net.down[, "interaction_name", drop = F]
gg2 <- netVisual_bubble(cellchat, pairLR.use = pairLR.use.down, sources.use = idents.Immune, targets.use = idents.Epi, comparison = c(1, 2),  angle.x = 90, remove.isolate = T,title.name = paste0("Down-regulated signaling in ", names(object.list)[2]))
#> Comparing communications on a merged object
gg1 + gg2
```
...
# Chord diagram
par(mfrow = c(1,2), xpd=TRUE)
netVisual_chord_gene(object.list[[2]], sources.use = idents.Epi, targets.use = idents.Immune, slot.name = 'net', net = net.up, lab.cex = 0.8, small.gap = 3.5, title.name = paste0("Up-regulated signaling in ", names(object.list)[2]))
#> Note: The first link end is drawn out of sector 'MIF'.
netVisual_chord_gene(object.list[[1]], sources.use = idents.Epi, targets.use = idents.Immune, slot.name = 'net', net = net.down, lab.cex = 0.8, small.gap = 3.5, title.name = paste0("Down-regulated signaling in ", names(object.list)[2]))
```

```{r fig.width=16,fig.height=8}
# Chord diagram
par(mfrow = c(1,2), xpd=TRUE)
netVisual_chord_gene(object.list[[2]], sources.use = idents.Immune, targets.use = idents.Epi, slot.name = 'net', net = net.up, lab.cex = 0.8, small.gap = 3.5, title.name = paste0("Up-regulated signaling in ", names(object.list)[2]))
#> Note: The first link end is drawn out of sector 'MIF'.
netVisual_chord_gene(object.list[[1]], sources.use = idents.Immune, targets.use = idents.Epi, slot.name = 'net', net = net.down, lab.cex = 0.8, small.gap = 3.5, title.name = paste0("Down-regulated signaling in ", names(object.list)[2]))
```
...
for (i in 1:length(object.list)) {
  netVisual_chord_gene(object.list[[i]], sources.use = idents.Epi, targets.use = idents.Immune, lab.cex = 0.5, title.name = paste0("Signaling from Epi - ", names(object.list)[i]))
}
```
...
# compare all the interactions sending from Inflam.FIB to DC cells
for (i in 1:length(object.list)) {
  netVisual_chord_gene(object.list[[i]], sources.use = idents.Immune, targets.use = idents.Epi, lab.cex = 0.5, title.name = paste0("Signaling from Immune - ", names(object.list)[i]))
}
```
...
par(mfrow = c(1, 2), xpd=TRUE)
for (i in 1:length(object.list)) {
  netVisual_chord_gene(object.list[[i]], sources.use = idents.Epi, targets.use = idents.Immune,slot.name = "netP", title.name = paste0("Signaling pathways sending from Epi - ", names(object.list)[i]), legend.pos.x = 10)
}
```
...
par(mfrow = c(1, 2), xpd=TRUE)
for (i in 1:length(object.list)) {
  netVisual_chord_gene(object.list[[i]], sources.use = idents.Immune, targets.use = idents.Epi,slot.name = "netP", title.name = paste0("Signaling pathways sending from Immune - ", names(object.list)[i]), legend.pos.x = 10)
}
```
...
```{r}
cellchat@meta$datasets = factor(cellchat@meta$datasets, levels = c("LI", "SI")) # set factor level
...
```{r}
saveRDS(cellchat, file = "cellchat_comparisonAnalysis_Mouse_Large_Intestine_vs_Small_Intestine.rds")
sessionInfo()
```
