...
```{r}
Results.dir <- "./4.2-Downstream-Analysis/DEGs"
Rds.dir <- "./4.1-Replicates-Merged-Cells-Rds"
Resolution <- "0.6"
Assay.used <- "CellBender"
Group.by <- "organ"
...
```{r}
# # add batch info which will be used DEGs analysis
# cds[["batch"]] <- gsub("APCmin_Mouse_Colon_Tumor_Mesen_","",gsub("^APCmin_Mouse_Colon_Para_Mesen_","",cds$orig.ident))
# cds[["batch"]] <- as.numeric(ifelse(cds[["batch"]] == "210608",1,2  ))
```
...
```{r}
cds$celltype.stim <- paste(cds@meta.data[,Group.by],Idents(cds), sep = "_")
cds$celltype <- Idents(cds)
Idents(cds) <- "celltype.stim"
...
for (i in (1:length(levels(cds$celltype)))) {
  ...
  if (sum(Idents(cds) %in% samplesSelected[1]) < 3 | sum(Idents(cds) %in% samplesSelected[2]) < 3) {
  ...
  write.csv(b.interferon.response, file = gsub(" ","-", paste(Results.dir,"/DEGs-of-cluster-",i-1,"-",samplesSelected[2],"-vs-",samplesSelected[1],".By.MAST.csv",sep = "")))
  ...
  if (length(Meta.to.regress) !=0 ) {
    ...
    write.csv(b.interferon.response, file = gsub(" ","-", paste(Results.dir,"/DEGs-of-cluster-",i-1,"-",samplesSelected[2],"-vs-",samplesSelected[1],".By.MAST.",paste(Meta.to.regress,collapse = "."),".Regressd.csv",sep = "")))
