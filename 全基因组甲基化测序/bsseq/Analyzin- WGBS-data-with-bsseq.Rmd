...
The bsseqData contains a script, ``inst/script/create\_BS.cancer.R``, describing how this data is created from the Merman alignment output (also contained in the package). Note that the current version of the BSmooth pipeline uses a slightly different alignment output format.
...
# # Error in SimpleList(coef = ans_coef) : object 'ans_coef' not found
```
...
(the BS.cancer.ex.tstat is also available for direct inspection in the bsseqData package.)
