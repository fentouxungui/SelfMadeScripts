function (object, assay = NULL, binarizeMethod = "median", FoldChange = 2, 
    p.adj = 0.05, cores = 1) 
{
    if (is.null(assay)) {
    ...
    gs <- sortGenes(expr_mtx, Idents(object), binarizeMethod = binarizeMethod, 
        cores = cores)
    ...
    for (i in 1:ncol(gene_adj.p)) {
        ...
        if (length(temp_gene) != 0) {
            ...
            temp_cluster_cells <- WhichCells(object = object, 
                idents = temp_cluster)
            temp_cluster_mtx <- expr_mtx[temp_gene, temp_cluster_cells]
            temp_other_cells <- WhichCells(object = object, idents = colnames(gene_adj.p)[colnames(gene_adj.p) != 
                temp_cluster])
            ...
            if (length(temp_gene) > 1) {
                ...
                temp_gene_sort <- names(sort(gene_score[temp_gene_filter, 
                  i], decreasing = T))
            }
            else {
            ...
            if (!is.null(temp_gene_sort)) {
                temp_matrix <- data.frame(Marker = temp_gene_sort, 
                  Cluster = colnames(gene_adj.p)[i], FoldChange = temp_fc_filter, 
                  P.value = gene_pvalue[temp_gene_sort, i], P.adj = gene_adj.p[temp_gene_sort, 
                    i], Gene.Score = gene_score[temp_gene_sort, 
                    i])
